        ...
        if sort_key in valid_sort_keys:
            ...
            # --- 期限 (due_date) によるソート処理 (NULL値の扱いを制御) ---
                ...
                if order == "asc":
                    ...
                    queryset = queryset.order_by(
                        "due_date_null", "due_date", "-created_at"
                    )
                else:
        ...
        if current_category_filter and current_category_filter.isdigit():
            try:
                context["selected_category"] = Category.objects.get(
                    pk=current_category_filter, user=self.request.user
                )
...
class TaskUpdateStatusView(LoginRequiredMixin, View):
    ...
    def post(self, request, pk):
        ...
        filter_params = request.POST.get(
            "filter_params", ""
        )  # リダイレクト時に元のフィルタを復元するため
...
class TrashView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        return Task.objects.filter(user=self.request.user, is_deleted=True).order_by(
            "-deleted_at"
        )  # 削除日時が新しい順
...
class TaskBulkDeleteView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        queryset = Task.objects.filter(user=request.user, is_deleted=True)
...
class CategoryCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        if Category.objects.filter(
            user=self.request.user, name=form.cleaned_data["name"]
        ).exists():
...
class CategoryUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        if (
            Category.objects.filter(user=self.request.user, name=new_name)
